...
int Demo_AlarmFortify()
{
	...
	memcpy(struLoginInfo.sDeviceAddress, "192.168.5.103", NET_DVR_DEV_ADDRESS_MAX_LEN);
	memcpy(struLoginInfo.sUserName, "admin", NAME_LEN);
	memcpy(struLoginInfo.sPassword, "147258369q", NAME_LEN);
...
int AlarmFortify(LONG lUserID, bool byRetVQDAlarmType)
{

	NET_DVR_SETUPALARM_PARAM_V50 struSetupAlarmParam = {0};
	struSetupAlarmParam.dwSize = sizeof(struSetupAlarmParam);
	if (byRetVQDAlarmType)
	{
		struSetupAlarmParam.byRetVQDAlarmType = TRUE; // Prefer VQD Alarm type of NET_DVR_VQD_ALARM
	}
	else
	{
		struSetupAlarmParam.byRetVQDAlarmType = FALSE; // Prefer VQD Alarm type of NET_DVR_VQD_ALARM
	}

	return Demo_AlarmFortify();
}